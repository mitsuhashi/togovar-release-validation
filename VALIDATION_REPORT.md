...
2026年9月10日時点で、全1,974比較のうち553比較（28.0%）が正常終了している。
正規化済みallele keyの集合差`old - new`は、完了済み範囲ではいずれも0件だった。
...
逐次実行として再開したCWLプロセスは稼働中だが、確認時点では比較用の子プロセスが
起動しておらず、完了比較数は553件から増えていない。最後に比較結果が更新されたのは
2026年9月9日06:46（JST）である。したがって、現在の実進捗は553/1,974比較のままであり、
新しい比較jobの開始またはキャッシュ更新を確認できるまでは、処理が前進しているとは
判定しない。
