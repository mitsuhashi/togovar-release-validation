...
2026年9月10日時点で、全1,974比較のうち555比較（28.1%）が正常終了している。
このうちGRCh37 `jga_snp`では、正規化済みallele keyの集合差`old - new`が121件検出された。
それ以外の553比較では欠落alleleは0件だった。
...
大規模scatterで停止した逐次実行の代わりに、dataset単位のバッチ再開を使用している。
GRCh37 `jga_snp`の1比較は2026年9月10日に完了し、旧・新とも正規化後のallele数は
1,249,724件、旧版にのみ存在するalleleは121件、参照不一致は旧・新とも0件だった。
同日、`jga_wes`の1比較も完了し、旧・新とも4,679,025件、旧版にのみ存在するalleleは
0件、参照不一致は旧・新とも0件だった。
残りの比較も同じバッチ方式で実行する。
